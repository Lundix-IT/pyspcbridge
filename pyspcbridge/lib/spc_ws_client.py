...
class SpcWsClient():
    ...
    def __init__(self, bridge, gw_ip_address, gw_port, credentials, loop, session, http_client):
    ...
    def ws_start(self):
        ...
        _LOGGER.debug("Start websocket communication with SPC Bridge: %s:%i", self._gw_ip_address, self._gw_port)
        proto = "ws"
        params = ""
        if len(self._credentials["ws_username"]) and len(self._credentials["ws_password"]):
            proto = "wss"
    ...
    def ws_stop(self):
        ...
        _LOGGER.debug("Stop websocket communication with SPC Bridge: %s:%i", self._gw_ip_address, self._gw_port)
    ...
    def _handle_area_event(self, event):
        ...
        if area_id > 0:
            if sia_code == "OG":       # Unset
                ...
                self._set_value("area", area_id, {"mode": 0, "unset_user": username, "changed_by": username, "pending_exit": False})
                self._set_value("panel", None, {"changed_by": username})
                return
            elif sia_code == "NL":
                if event_id == 3502:   # Partset A
                    self._set_value("area", area_id, {"mode": 1, "changed_by": username, "pending_exit": False})
                    self._set_value("panel", None, {"changed_by": username})
                    return
                elif event_id == 3503: # Partset B
                    self._set_value("area", area_id, {"mode": 2, "changed_by": username, "pending_exit": False})
                    self._set_value("panel", None, {"changed_by": username})
                    return
            elif sia_code == "CG":     # Fullset
                self._set_value("area", area_id, {"mode": 3, "set_user": username, "changed_by": username, "pending_exit": False})
                self._set_value("panel", None, {"changed_by": username})
                return
    ...
    async def _async_handle_zone_event(self, event):
        ...
        if ((event_id >= 1000 and event_id <= 1029) or
            (event_id >= 1100 and event_id <= 1129) or
            (event_id >= 1200 and event_id <= 1223) or
            (event_id >= 1300 and event_id <= 1323) or
            (event_id >= 1400 and event_id <= 1423) or
            (event_id >= 1500 and event_id <= 1523) or
            (event_id >= 1600 and event_id <= 1623) or
            (event_id >= 1700 and event_id <= 1723) or
            (event_id >= 1800 and event_id <= 1823) or
            (event_id >= 1900 and event_id <= 1923) or
            (event_id >= 2000 and event_id <= 2023) or
            (event_id >= 2100 and event_id <= 2117) or
            (event_id >= 8004 and event_id <= 8005)):
            ...
            if zone_id > 0:
                try:
                    ...
                    if data and type(data) == list:
                        ...
                        if id > 0:
                            self._set_value('zone', id, { "input": input, "status": status });
                            return True
    ...
    async def _async_ws_handler(self, data):
        ...
        # Door events
        if (event_id == 3000 or event_id == 3001 or event_id == 3002 or
            event_id == 3003 or event_id == 3007 or event_id == 3008 or
            event_id == 3009):
        ...
        # Area events
        if (sia_code == "CI" or sia_code == "CG" or
            sia_code == "OG" or sia_code == "NL"):
